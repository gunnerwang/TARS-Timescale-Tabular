...
from model.lib.tabm.deep import Identity
...
class MLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        d_first = d_layers[0] if d_in is None else d_in
        self.blocks = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(d_first if i == 0 else d_layers[i - 1], d_layers[i]),
        ...
        self.output = None if d_out is None else nn.Linear(d_layers[-1], d_out)
