...
class Method(object, metaclass=abc.ABCMeta):
    ...
    def metric(self, predictions, labels, y_info):
        ...
        if self.is_regression:
            ...
            return (auc, accuracy, avg_recall, avg_precision, f1_score, log_loss), ("AUC", "Accuracy", "Avg_Recall", "Avg_Precision", "F1", "LogLoss")
            ...
            log_loss = skm.log_loss(labels, predictions, labels=range(self.d_out))
            auc = skm.roc_auc_score(labels, predictions, labels=range(self.d_out), average='macro', multi_class='ovr')
            return (auc, accuracy, avg_recall, avg_precision, f1_score, log_loss), ("AUC", "Accuracy", "Avg_Recall", "Avg_Precision", "F1", "LogLoss")
        else:
