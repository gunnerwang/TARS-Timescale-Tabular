...
DATASET_SIZE = 340596     # ct: 227087, de: 279415, mr: 160019, we: 340596
BATCH_SIZE = 1024
...
class MultiTimescaleImplicitEncoder(nn.Module):
    ...
    def __init__(self, d_in, d_out, dataset_size=DATASET_SIZE, batch_size=BATCH_SIZE, 
        ...
        print(f"🔧 Adaptive Warm-up Configuration for {self.warmup_params['dataset_category']} dataset:")
...
class TemporalComponentRouter(nn.Module):
    ...
    def __init__(self, fast_dim, slow_dim, ultra_dim, fusion_strategy='softmax', 
        ...
        print(f"🌡️  Temperature Annealing: {warmup_params['temperature_annealing_epochs']:.1f} epochs ({self.temperature_warmup_steps} steps)")
        if enable_bypass:
            print(f"🔄 Bypass Mode: Enabled (smoothing factor: {bypass_smoothing})")
...
class MLP_Temporal(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.layers = nn.ModuleList(
            [
                nn.Linear(d_layers[i - 1] if i else self.total_dim, x)
                for i, x in enumerate(d_layers)
            ]
        )
        self.head = nn.Linear(d_layers[-1] if d_layers else self.total_dim, self.d_out)
        
        # Initialize CBP layers if enabled
        self.cbp_layers = nn.ModuleList()
        if self.use_cbp and len(d_layers) > 0:
            print(f"🔄 CBP (Continual Backpropagation) ENABLED")
            ...
            print(f"   └── Total CBP Layers: {len(d_layers)} (hidden-to-hidden + hidden-to-head)")
            
            # Create CBP layers for consecutive hidden layers
            for i in range(len(self.layers) - 1):
                in_layer = self.layers[i]
                out_layer = self.layers[i + 1]
                # Type cast since we know these are nn.Linear layers
                ...
                print(f"      CBP Layer {i+1}: {in_layer.out_features} → {out_layer.out_features} neurons")
            
            # Create CBP layer for last hidden layer to head
            if len(self.layers) > 0:
                last_layer = self.layers[-1]
                # Type cast since we know this is an nn.Linear layer
                ...
                print(f"      CBP Head: {self.layers[-1].out_features} → {self.head.out_features} neurons")
        elif self.use_cbp and len(d_layers) == 0:
            print("⚠️  CBP enabled but no hidden layers found - CBP will not be used")
        else:
            print("📋 Using standard MLP (CBP disabled)")
        
        # Print configuration summary
        print(f"\n🎯 Temporal-PLR-MLP Configuration Summary:")
    ...
    def forward(self, x_num, x_cat, idx):
            ...
            if hasattr(self, 'temporal_router') and self.implicit_time_dim > 0 and drift_encoding_dict is not None:
                ...
                if self.training:
                    ...
                    if self._routing_log_step % 1000 == 0:  # Log every 1000 steps
                        ...
                        print(f"📊 Routing Stats (Step {self._routing_log_step}):")
        ...
        # Final x_num already contains processed features
        x_num_fused = x_num
        
        # Combine all features (temporal embeddings are already included in x_num_fused after PLR processing)
        if x_num_fused is not None and x_cat is not None:
            x = torch.cat([x_num_fused, x_cat], dim=-1)
        elif x_num_fused is not None:
            x = x_num_fused
        elif x_cat is not None:
            x = x_cat.to(torch.float32)
        else:
            # This shouldn't happen in normal cases, but handle gracefully
            batch_size = idx.shape[0] if idx is not None else 1
            device = idx.device if idx is not None else torch.device('cpu')
            x = torch.zeros(batch_size, self.total_dim, device=device)
        
        if self.use_cbp and len(self.cbp_layers) > 0:
            # Print CBP usage info only once during first training forward pass
            if self.training and not hasattr(self, '_cbp_info_printed'):
                print("🚀 CBP forward pass active - monitoring neuron utility and performing selective replacement")
                self._cbp_info_printed = True
                
            # Forward pass with CBP layers - CBP is called after activations
            for i, layer in enumerate(self.layers):
                ...
                # Apply CBP layer after activation (except for the last layer)
                if i < len(self.cbp_layers) - 1:  # -1 because last CBP layer is for head
                    x = self.cbp_layers[i](x)
            
            # Apply final layer (head) and its CBP
            logit = self.head(x)
            if len(self.cbp_layers) > 0:  # Apply final CBP layer for head
                logit = self.cbp_layers[-1](logit)
        else:
            # Standard forward pass without CBP
    ...
    def set_test_time_adaptation(self, enable: bool):
        ...
        print(f"🔄 Test-time Adaptation: {mode_str}")
        if enable:
