...
DATASET_SIZE = 279415     # ct: 227087, de: 279415, mr: 160019, we: 340596
BATCH_SIZE = 1024
...
class MLP_Temporal(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Total input dimension includes features and integrated temporal embeddings
        # Use actual temporal dimension based on whether router is used
        if self.implicit_time_dim > 0 and hasattr(self, 'temporal_embeddings'):
            ...
            actual_temporal_dim = temporal_dim
        self.total_dim = self.d_in + actual_temporal_dim
        
        # Add fusion gate for temporal attention
        if temporal_dim > 0:
            ...
            self.fusion_gate = nn.Linear(fusion_input_dim, d_in)
        ...
        print(f"\n🎯 Temporal-MLP Configuration Summary:")
        print(f"   ├── Drift Significance Threshold: {drift_significance_threshold}")
    ...
    def forward(self, x, x_cat, idx):
        # Generate multi-timescale drift encoding (structured output)
        if self.implicit_time_dim > 0:
            drift_encoding_dict = self.implicit_time_encoder(x)
            drift_significant = drift_encoding_dict.get('drift_significant', True)
        ...
        if hasattr(self, 'temporal_embeddings'):
            ...
            # Apply adaptive fusion with features if fusion gate exists
            if hasattr(self, 'fusion_gate'):
                temporal_weights = torch.sigmoid(self.fusion_gate(temporal_emb))
                x_weighted = x * temporal_weights
                x_fused = x_weighted + x * 0.2  # Residual connection
                x = torch.cat([x_fused, temporal_emb], dim=-1)
            else:
                x = torch.cat([x, temporal_emb], dim=-1)
