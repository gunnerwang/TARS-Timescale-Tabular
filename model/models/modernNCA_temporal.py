...
class CBP_Residual_block(nn.Module):
    """CBP-enabled Residual block for ModernNCA with continuous neuron replacement"""
    def __init__(self, d_in, d, dropout, replacement_rate=1e-4, maturity_threshold=100, cbp_init='kaiming'):
        super().__init__()
        # Standard layers (same as original Residual_block)
        ...
        # CBP layer for the two linear layers in residual block
        self.cbp_layer = CBPLinear(
            in_layer=self.linear0,
            out_layer=self.linear1,
    ...
    def forward(self, x):
        ...
        z = self.activation(z)
        z = self.dropout(z)
        # Apply CBP monitoring after dropout
        z = self.cbp_layer(z)
        z = self.linear1(z)
        return z
...
class MultiTimescaleImplicitEncoder(nn.Module):
    ...
    def __init__(self, d_in, d_out, dataset_size=DATASET_SIZE, batch_size=BATCH_SIZE, 
        ...
        print(f"🔧 NCA Adaptive Warm-up Configuration for {self.warmup_params['dataset_category']} dataset:")
...
class TemporalComponentRouter(nn.Module):
    ...
    def __init__(self, fast_dim, slow_dim, ultra_dim, fusion_strategy='softmax', 
        ...
        print(f"🌡️  NCA Temperature Annealing: {warmup_params['temperature_annealing_epochs']:.1f} epochs ({self.temperature_warmup_steps} steps)")
        if enable_bypass:
            print(f"🔄 NCA Bypass Mode: Enabled (smoothing factor: {bypass_smoothing})")
...
class ModernNCA_Temporal(nn.Module):
    def __init__(
        ...
        *,
        ...
        temporal_embeddings: Optional[Dict[str, Any]],
        ...
        self.T = temperature  # NCA temperature parameter (different from router temperature)
        self.sample_rate = sample_rate
        self.implicit_time_dim = implicit_time_dim
        self.use_cbp = use_cbp
        self.replacement_rate = replacement_rate
        self.maturity_threshold = maturity_threshold
        self.cbp_init = cbp_init
        ...
        # Handle num_embeddings integration with temporal features (following original PLR logic)
        # PLR should be applied AFTER concatenating temporal embeddings with numerical features
        if num_embeddings is not None:
            # PLR input dimension = d_num + temporal_dim (after concatenation)
            if hasattr(self, 'temporal_embeddings'):
                if self.implicit_time_dim > 0 and hasattr(self, 'temporal_router'):
                    ...
                    plr_input_dim = self.d_num + self.temporal_embeddings.component_dim
                else:
                    # Fallback mode: use full temporal embedding dimension
                    plr_input_dim = self.d_num + temporal_dim
            else:
                plr_input_dim = self.d_num
                
            self.num_embeddings = make_module(num_embeddings, n_features=plr_input_dim)
            # After PLR embedding: (d_num + temporal_dim) features become (d_num + temporal_dim) * d_embedding features
            embedded_d_num = plr_input_dim * num_embeddings['d_embedding']
        else:
            self.num_embeddings = None
            # Without PLR: just d_num + temporal_dim
            if hasattr(self, 'temporal_embeddings'):
                if self.implicit_time_dim > 0 and hasattr(self, 'temporal_router'):
                    embedded_d_num = self.d_num + self.temporal_embeddings.component_dim
                else:
                    embedded_d_num = self.d_num + temporal_dim
            else:
                embedded_d_num = self.d_num
                
        # Calculate total dimension: embedded numerical features (already includes temporal) + categorical features
        # Note: embedded_d_num already includes temporal features after concatenation and PLR processing
        categorical_dim = d_in - d_num  # Categorical features dimension
        self.total_dim = embedded_d_num + categorical_dim
        
        # Add fusion gate for adaptive temporal-numerical feature interaction
        if temporal_dim > 0:
            # Choose fusion gate input dimension based on whether router is used
            if self.implicit_time_dim > 0 and hasattr(self, 'temporal_embeddings'):
                # Router mode: use component dimension (weighted sum output)
                fusion_input_dim = self.temporal_embeddings.component_dim
            else:
                # Fallback mode: use full temporal embedding dimension
                fusion_input_dim = temporal_dim
            self.fusion_gate = nn.Linear(fusion_input_dim, self.d_num)
        
        # NCA-specific layers (preserve original ModernNCA structure)
        if self.n_blocks > 0:
        ...
        self.encoder = nn.Linear(self.total_dim, self.dim)
        
        # CBP is now integrated into residual blocks themselves
        if self.use_cbp and self.n_blocks > 0:
            print(f"🔄 NCA CBP (Continual Backpropagation) ENABLED")
            ...
            print(f"   └── Total CBP Residual Blocks: {n_blocks}")
            print(f"      └── Each residual block contains CBP monitoring for its linear layers")
                
        elif self.use_cbp and self.n_blocks == 0:
            print("⚠️  CBP enabled but no residual blocks found - CBP will not be used")
        else:
            print("📋 Using standard NCA residual blocks (CBP disabled)")
        
        # Print configuration summary
        print(f"\n🎯 Temporal-ModernNCA Configuration Summary:")
        print(f"   ├── Numerical Features: {d_num}")
        if hasattr(self, 'temporal_embeddings'):
            if self.implicit_time_dim > 0 and hasattr(self, 'temporal_router'):
            ...
            if self.num_embeddings is not None:
                print(f"   ├── PLR Input: {d_num} + temporal → {embedded_d_num} (after PLR embedding)")
                ...
                print(f"   ├── Combined Num+Temporal: {embedded_d_num} (no PLR)")
                if hasattr(self, 'fusion_gate'):
                    print(f"   ├── Fusion Gate: Enabled (adaptive temporal-numerical interaction)")
        else:
            print(f"   ├── No Temporal Features")
            if self.num_embeddings is not None:
                print(f"   ├── PLR Input: {d_num} → {embedded_d_num} (after PLR embedding)")
        print(f"   ├── Categorical Features: {categorical_dim}")
        print(f"   ├── Total Input Dimension: {self.total_dim}")
        print(f"   ├── NCA Encoder Dimension: {self.dim}")
        print(f"   ├── NCA Residual Blocks: {self.n_blocks}")
        print(f"   ├── NCA Temperature: {self.T}")
        print(f"   ├── NCA Sample Rate: {self.sample_rate}")
        print(f"   ├── Drift Significance Threshold: {drift_significance_threshold}")
    ...
    def make_layer(self):
        if self.use_cbp:
            # Use CBP-enabled residual block
            block = CBP_Residual_block(
                self.dim, 
                self.d_block, 
                self.dropout,
                replacement_rate=self.replacement_rate,
                maturity_threshold=self.maturity_threshold,
                cbp_init=self.cbp_init
            )
        else:
    ...
    def forward(self, x, y, idx, candidate_x, candidate_y, candidate_idx, is_train):
        # NCA-specific logic: handle candidate sampling for training
        if is_train:
        ...
        if self.implicit_time_dim > 0 and x_num is not None:
            ...
            # Also compute for candidate data (needed for consistent processing)
            candidate_drift_encoding_dict = self.implicit_time_encoder(candidate_x_num)
        else:
            drift_encoding_dict = None
            candidate_drift_encoding_dict = None
            drift_significant = True  # Default to drift-based routing if no encoder
            drift_magnitude = 0.0
        
        # Generate temporal embeddings using component routing FIRST (following original PLR logic)
        if hasattr(self, 'temporal_embeddings'):
            # Process query temporal embeddings
            if hasattr(self, 'temporal_router') and self.implicit_time_dim > 0 and drift_encoding_dict is not None:
                ...
                if self.training:
                    ...
                    if self._routing_log_step % 1000 == 0:  # Log every 1000 steps
                        ...
                        print(f"📊 NCA Routing Stats (Step {self._routing_log_step}):")
            ...
            # Process candidate temporal embeddings (same logic)
            if hasattr(self, 'temporal_router') and self.implicit_time_dim > 0 and candidate_drift_encoding_dict is not None:
                candidate_component_embeddings = self.temporal_embeddings.forward_components(candidate_idx)
                candidate_temporal_emb = self.temporal_router(candidate_drift_encoding_dict, candidate_component_embeddings)
            else:
                candidate_temporal_emb = self.temporal_embeddings(candidate_idx)
                
            # Apply adaptive fusion with numerical features if fusion gate exists (from mlp_temporal.py)
            if hasattr(self, 'fusion_gate') and x_num is not None:
                ...
                # Following original PLR logic: concat temporal embeddings with fused numerical features
                x_num = torch.cat([x_num_fused, temporal_emb], dim=-1)
                
                # Apply same fusion to candidate data
                candidate_temporal_weights = torch.sigmoid(self.fusion_gate(candidate_temporal_emb))
                candidate_x_num_weighted = candidate_x_num * candidate_temporal_weights
                candidate_x_num_fused = candidate_x_num_weighted + candidate_x_num * 0.2
                candidate_x_num = torch.cat([candidate_x_num_fused, candidate_temporal_emb], dim=-1)
            else:
                # Fallback: direct concatenation (original PLR logic)
                if x_num is not None:
                    x_num = torch.cat([x_num, temporal_emb], dim=-1)
                    candidate_x_num = torch.cat([candidate_x_num, candidate_temporal_emb], dim=-1)
        else:
            temporal_emb = None
            
        # Process numerical embeddings AFTER adding temporal information (original PLR logic)
        # This allows PLR to learn joint numerical-temporal representations
        if self.num_embeddings is not None and self.d_num > 0 and x_num is not None:
            x_num = self.num_embeddings(x_num).flatten(1)
            candidate_x_num = self.num_embeddings(candidate_x_num).flatten(1)
        
        # Combine all features (temporal embeddings are already included in x_num after PLR processing)
        x = torch.cat([x_num, x_cat], dim=-1)
        candidate_x = torch.cat([candidate_x_num, candidate_x_cat], dim=-1)
        
        # NCA-specific encoding (preserve original ModernNCA logic)
        # CBP is now handled inside the residual blocks automatically
        if self.n_blocks > 0:
        ...
        # NCA-specific logic: candidate preparation and distance computation
        if is_train:
        ...
        # NCA-specific distance computation and prediction
        distances = torch.cdist(x, candidate_x, p=2)
        distances = distances / self.T  # Apply NCA temperature (different from router temperature)
    ...
    def set_test_time_adaptation(self, enable: bool):
        ...
        print(f"🔄 NCA Test-time Adaptation: {mode_str}")
        if enable:
